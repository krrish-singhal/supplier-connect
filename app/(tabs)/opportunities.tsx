...
export default function OpportunitiesScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (opportunityFilters.status !== "all")
      o = o.filter((x) => x.status === opportunityFilters.status);
    if (debounced) {
    ...
    if (opportunityFilters.region !== "all")
      o = o.filter((x) => x.region === opportunityFilters.region);
    o.sort(
      (a, b) =>
  ...
  const countFor = (s: OpportunityStatus | "all") =>
    s === "all"
      ? allOpportunities.length
      : allOpportunities.filter((x) => x.status === s).length;
    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={["top", "bottom"]}>
      <View className="bg-white px-5 pt-3 pb-3.5 border-b border-slate-100">
        ...
        <SearchBar
          ...
          onChangeText={setOpportunitySearch}
        ...
        >
          {STATUS_TABS.map((tab) => {
            ...
            return (
              <TouchableOpacity
                key={tab.value}
                onPress={() => setOpportunityStatus(tab.value)}
                className={`flex-row items-center gap-2 px-3.5 py-2.5 rounded-3xl ${sel ? "bg-blue-600" : "bg-slate-100"}`}
              >
                <Text
                  className={`text-sm font-semibold ${sel ? "text-white" : "text-slate-600"}`}
                >
                  {tab.label}
                </Text>
                <View
                  className={`px-1.5 py-0.5 rounded-xl ${sel ? "bg-blue-700" : "bg-slate-200"}`}
                >
                  <Text
                    className={`text-[11px] font-bold ${sel ? "text-white" : "text-slate-500"}`}
                  >
                    {cnt}
                  </Text>
                </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            onAction={resetOpportunityFilters}
          />
        }
