...
export default function HomeScreen() {
  ...
  const { supplierFilters, setSupplierRegion, setSupplierSearch } =
    useFiltersStore();
  ...
  const fetchData = useCallback(async () => {
    try {
      const [suppRes, oppRes] = await Promise.all([
        ...
        api.get<{ opportunities: Opportunity[] }>("/api/opportunities", {
          limit: 10,
        }),
      ]);
  ...
  const recentOpportunities = useMemo(
    () => opportunities.filter((o) => o.status === "open").slice(0, 3),
    [opportunities],
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50" edges={["top", "bottom"]}>
      ...
      >
          ...
          <View className="flex-row gap-3">
            {STATS.map((stat) => (
              ...
              >
                ...
                <Text className="text-[22px] font-bold text-slate-900 mt-2">
                  {stat.value}
                </Text>
        ...
        {/* Region Tabs */}
        <View className="px-5">
          <Text className="text-[17px] font-bold text-slate-900 mb-1 mt-4">
            Browse by Region
          </Text>
        </View>
        <RegionTabs
